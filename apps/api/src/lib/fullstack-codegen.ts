    ...
    content: JSON.stringify(
      {
        ...
        scripts: {
          ...
          build: "vite build",
          // Production start (used when deployed): the server serves the built
          // dist/ + the API on one port (process.env.PORT).
...
`,
  ...
  {
    // Render Blueprint: one web service that builds the frontend and runs the
    // server (which serves dist/ + the API on $PORT). Free-tier deployable.
    path: "render.yaml",
    content: `services:
  - type: web
    name: omni-app
...
Generated by Omni. Vite + React frontend, Express + better-sqlite3 backend.
...
you, or connect this repo in the Render dashboard (build \`npm install && npm run
build\`, start \`npm start\`). Note: SQLite lives on the instance's disk, so data
resets on redeploy unless you attach a persistent disk.
`,
  },
];
...
const SYSTEM = [
  ...
  "- server/index.js (required): an Express app. Listen on process.env.PORT || " + API_PORT + ". Use better-sqlite3 with new Database(process.env.DATABASE_PATH || 'data.db'); create tables on startup if missing. Enable express.json(). Expose JSON REST endpoints under /api/*. FOR PRODUCTION, also serve the built frontend: import { existsSync } from 'node:fs'; if a 'dist' directory exists, call app.use(express.static('dist')) and add a fallback that returns dist/index.html for any non-/api GET request (guard it so it never intercepts /api routes). In development 'dist' is absent so this is a no-op and Vite serves the frontend.",
  "",
  "RULES:",
